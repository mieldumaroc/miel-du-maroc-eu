...
const CartDrawer = () => {
  ...
  const buildWhatsAppMessage = () => {
    const greetings = { fr: 'Bonjour, je voudrais commander:', de: 'Hallo, ich möchte bestellen:', nl: 'Hallo, ik wil graag bestellen:', ar: 'مرحباً، أريد طلب:', en: 'Hello, I would like to order:' };
    const totals = { fr: 'Total', de: 'Gesamt', nl: 'Totaal', ar: 'المجموع', en: 'Total' };
    const thanks = { fr: 'Merci!', de: 'Danke!', nl: 'Dank u!', ar: 'شكراً!', en: 'Thank you!' };
    const lang = language || 'en';
    let msg = (greetings[lang] || greetings.en) + '\n\n';
    items.forEach((item, i) => {
      const name = getItemName(item);
      const lineTotal = item.price * item.quantity;
      msg += `${i + 1}. ${name} (${item.size}) x${item.quantity} — ${lineTotal} MAD\n`;
    });
    msg += `\n${totals[lang] || totals.en}: ${totalPriceMAD} MAD`;
    msg += `\n\n${thanks[lang] || thanks.en}`;
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#E8E2D2]">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-heading text-xl font-medium text-[#1A1713]">
                  {language === 'fr' ? 'Panier' : language === 'de' ? 'Warenkorb' : language === 'nl' ? 'Winkelwagen' : language === 'ar' ? 'سلة التسوق' : 'Cart'}
              ...
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center" data-testid="cart-empty">
                  ...
                  <p className="text-[#5C5449] text-sm">
                    {language === 'fr' ? 'Votre panier est vide' : language === 'de' ? 'Ihr Warenkorb ist leer' : language === 'nl' ? 'Uw winkelwagen is leeg' : language === 'ar' ? 'سلة التسوق فارغة' : 'Your cart is empty'}
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  >
                    {language === 'fr' ? 'Vider le panier' : language === 'de' ? 'Warenkorb leeren' : language === 'nl' ? 'Winkelwagen legen' : language === 'ar' ? 'إفراغ السلة' : 'Clear cart'}
                  </button>
                </div>
              )}
            </div>
            ...
            {items.length > 0 && (
              <div className="border-t border-[#E8E2D2] px-6 py-5 space-y-4">
                ...
                <button
                  ...
                  className="w-full bg-[#25D366] text-white py-4 text-sm font-medium tracking-wide uppercase hover:bg-[#1DA851] transition-colors flex items-center justify-center gap-2"
                ...
                >
                  ...
                  {language === 'fr' ? 'Commander via WhatsApp' : language === 'de' ? 'Uber WhatsApp bestellen' : language === 'nl' ? 'Bestel via WhatsApp' : language === 'ar' ? 'اطلب عبر واتساب' : 'Checkout via WhatsApp'}
                </button>
              </div>
            )}
          </motion.div>
